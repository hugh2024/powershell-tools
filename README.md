...
A collection of PowerShell GUI utilities for Windows IT and security operations. Built for sysadmins and security engineers managing Windows environments.
...
| ------------------------------------------- | ------- | --------------------------------------------------------------------- |
| [NetProbe](#netprobe)                       | v1.0    | Nmap-based GUI network scanner for host, port, and traceroute testing |
...
**Version:** v1.0  |  **Requires:** PowerShell 5.1+, Nmap

A dark-themed PowerShell GUI tool for network scanning, host discovery, port enumeration, service detection, and traceroute testing using Nmap. Designed for personal labs, troubleshooting, and security testing without needing to memorize command-line switches.

Useful when you need to quickly test a host, scan a subnet, compare scan types, or run traceroute from a simple Windows GUI.

### What it does

* Scans a single IP, hostname, or CIDR range
* Supports TCP Connect, SYN, UDP, ACK, Service Detection, OS Detection, and Aggressive scan modes
* Includes optional host discovery settings and traceroute integration
* Works with either portable Nmap or system-installed Nmap
* Supports timeout handling and stopping a running scan from the GUI
* Keeps scan history visible in the interface for quick review

### Key Features

| Feature                | Details                                                  |
| ---------------------- | -------------------------------------------------------- |
| **Target Input**       | IP address, hostname, or CIDR range                      |
| **Scan Types**         | TCP Connect, SYN, UDP, ACK                               |
| **Advanced Detection** | Service version detection, OS detection, aggressive scan |
| **Traceroute**         | Built-in traceroute option from the GUI                  |
| **Timeout Control**    | Prevents scans from hanging indefinitely                 |
| **Stop Control**       | Stop an in-progress scan safely                          |
| **History**            | Keeps recent scan output visible in the interface        |
| **Nmap Support**       | Portable `./nmap/nmap.exe` or system PATH install        |
...
* Some scan types may require Administrator privileges depending on Nmap installation and selected options
* TCP Connect scan is the safest default when not running elevated
...
| Requirement           |   NetProbe  |   ADLockoutManager  |  LocalAdminAuditor  | NetworkSpeedChecker | ServerHealthCheck | SecurityChecker |   AppHunter Pro   |
| --------------------- | :---------: | :-----------------: | :-----------------: | :-----------------: | :---------------: | :-------------: | :---------------: |
| PowerShell 5.1+       |     Yes     |         Yes         |         Yes         |         Yes         |        Yes        |       Yes       |        Yes        |
| Run as Administrator  | Recommended |     Recommended     |         Yes         |     Recommended     |        Yes        |       Yes       |    Recommended    |
| Nmap                  |     Yes     |          No         |          No         |          No         |         No        |        No       |         No        |
| RSAT / AD Module      |      No     | Yes (auto-installs) | Yes (auto-installs) |          No         |         No        |        No       | Domain scope only |
| WinRM on this machine |      No     |          No         |  Yes (auto-enables) |         Yes         |        Yes        |        No       |   Remote queries  |
| WinRM on targets      |      No     |          No         |         Yes         |         Yes         |        Yes        |        No       |   Remote queries  |
| Domain membership     |      No     |         Yes         |         Yes         |          No         |         No        |        No       | Domain scope only |
